...
const setEmployeeData = async (empdata, cb) => {
  let data = await axios.post(
    "http://localhost:8000/api/v1/setemployee",
    empdata
  );
...
const updateTaskData = async (taskdata, cb) => {
  let data = await axios.put(
    `http://localhost:8000/api/v1/updatetask/${taskdata._id}`,
    taskdata
  );
  if (data.data) {
    cb(data.data);
...
const getEmpTaskData = async (taskdata, cb) => {
  let data = await axios.get(
    "http://localhost:8000/api/v1/getemployeetask/" + taskdata,
    taskdata
  );
  if (data.data) {
...
const updateEmployeeTaskData = async (taskdataX, cb) => {
  let data = await axios.put(
    `http://localhost:8000/api/v1/updatetask/${JSON.parse(
